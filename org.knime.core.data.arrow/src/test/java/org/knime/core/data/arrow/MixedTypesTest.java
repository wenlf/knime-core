		...
		try (final TableStore store = cache(createStore(chunkSize, DoubleType.INSTANCE, DoubleType.INSTANCE,
			...
			try (RowWriteCursor writeCursor = writeTable.getCursor()) {
				...
				for (int i = 0; i < numRows; i++) {
					...
					if (i % 100 == 0) {
						for (int j = 0; j < 2; j++) {
							doubleRange.getWriteValue(j).setMissing();
						}
					} else {
						for (int j = 0; j < 2; j++) {
							doubleRange.getWriteValue(j).setDouble(i + j);
						}
					}
				}
			}
			...
			try (RowReadCursor readCursor = readTable.newCursor()) {
				final ReadValueRange<DoubleReadValue> doubleRange = readCursor.getRange(0, 2);
				...
				for (int i = 0; i < numRows; i++) {
					...
					if (i % 100 == 0) {
						for (int j = 0; j < 2; j++) {
							Assert.assertTrue(doubleRange.getReadValue(j).isMissing());
						}
					} else {
						for (int j = 0; j < 2; j++) {
							Assert.assertEquals(i + j, doubleRange.getReadValue(j).getDouble(), 0.000001);
						}
					}
				}
			}
